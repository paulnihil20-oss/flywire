...
async function request(path,{method='GET',body,token,prefer,useSession=true}={}){
  if(!configReady())throw new Error('Online accounts are not configured yet.');
  const bearer=token&&token!==SUPABASE_ANON_KEY?token:(useSession?session?.access_token:null)||(SUPABASE_ANON_KEY.startsWith('eyJ')?SUPABASE_ANON_KEY:null),headers={apikey:SUPABASE_ANON_KEY,'Content-Type':'application/json',...(bearer?{Authorization:`Bearer ${bearer}`}:{ }),...(prefer?{Prefer:prefer}:{})};
...
export async function signUp({email,password,displayName}){const data=await request('/auth/v1/signup',{method:'POST',body:{email,password,data:{display_name:displayName}} ,token:SUPABASE_ANON_KEY,useSession:false});if(data.access_token)saveSession(data);return data;}
export async function signIn({email,password}){const data=await request('/auth/v1/token?grant_type=password',{method:'POST',body:{email,password},token:SUPABASE_ANON_KEY,useSession:false});saveSession(data);return data;}
